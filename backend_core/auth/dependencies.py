...
def get_tenant_optional(
    credentials: Annotated[Optional[HTTPAuthorizationCredentials], Depends(bearer_scheme)],
    x_api_key: Optional[str] = Header(default=None),
    brand_slug: Optional[str] = Header(default=None, alias="X-Brand-Slug"),
    store_id: Optional[str] = Header(default=None, alias="X-Store-Id"),
    db: Session = Depends(get_db),
    settings: Settings = Depends(get_settings),
) -> TenantContext:
    ...
    verify_dashboard_api_key(x_api_key, settings)
    slug = brand_slug or settings.brand_slug
    ext_store = store_id or settings.store_id
