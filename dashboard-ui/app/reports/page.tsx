  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const poll = async () => {
        ...
        setJobs((prev) => [job, ...prev.filter((j) => j.job_id !== job_id)]);
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-2">
            {jobs.length === 0 ? (
              <p className="text-sm text-muted-foreground">No reports generated this session.</p>
            ) : (
              jobs.map((job) => (
                <div key={job.job_id} className="flex items-center justify-between rounded-lg border border-border px-3 py-2">
                  <div>
                    <p className="text-sm font-medium">{job.report_type} · {job.output_format}</p>
            ...
            {!data?.length ? (
              ...
              data.map((s) => (
                <div key={s.id} className="rounded-lg border border-border px-3 py-2">
                  <p className="text-sm font-medium">{s.report_type} · {s.output_format}</p>
